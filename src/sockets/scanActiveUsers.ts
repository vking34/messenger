...
export default (_io: Server, socket: Socket) => {
    ...
    messengerNamespace.in(user_id).clients((_e, senderConnections) => {
    ...
    RoomModel.find(condition, projection, (_e, roomRecords) => {
        if (roomRecords) {
            ...
            roomRecords.forEach((room, index) => {
                ...
                messengerNamespace.in(room[target]).clients((_e, receiverConnections) => {
                    ...
                    if (index === roomRecords.length - 1)
                        messengerNamespace.to(user_id).emit(ACTIVE_USER_LIST_EVENT, data);
                });
            });
        }
    }).sort(sortOptions);
